...
const char polor_axis_codes[NUM_AXIS] = {'S', 'R', 'H', 'E'};
float feedrate_mm_m = 1500.0, saved_feedrate_mm_m;
...
void get_current_pos_polor(float polor[], float pos[])
{
    float angle[3];
    double x, y, z;
...
void line_to_destination_angle()
{
    float target[NUM_AXIS];
    LOOP_XYZ(i) target[i] = angle_destination[i];
    
    target[E_AXIS] = destination[E_AXIS];
...
char inverse_kinematics(const float in_cartesian[3], float angle[3]) {
    ...
    xIn = (x / sin(angleRot / MATH_TRANS) - MATH_L2 - get_front_end_offset()) / MATH_LOWER_ARM;///(x / cos(angleRot / MATH_TRANS) - MATH_L2 - MATH_FRONT_HEADER) / MATH_LOWER_ARM;
    
    
    
    phi = atan(zIn / xIn) * MATH_TRANS;//phi is the angle of line (from joint 2 to joint 4) with the horizon
    
    sqrtZX = sqrt(zIn*zIn + xIn*xIn); // ??2???4???
